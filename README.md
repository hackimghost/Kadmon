...
[![Focus](https://img.shields.io/badge/Focus-APT_%7C_Hardware_Hacking-darkred?style=for-the-badge)](https://github.com/hackimghost/Kadmon)
[![License](https://img.shields.io/badge/License-Proprietary-blue?style=for-the-badge)](https://github.com/hackimghost/Kadmon)
...
graph TD
    ...
    %% Entradas Físicas
    subgraph Hardware_Infraestrutura [Camada de Hardware e Infra]
        RF[Espectro RF / SDR]:::hardware
        FW[Firmware / Circuitos]:::hardware
        DC[Datacenters Offshore]:::hardware
    end

    %% Entradas Digitais e OSINT
    subgraph Coleta_Omnicamada [Coleta Web e Inteligência]
        SW[Surface / Deep / Dark Web]:::internet
        CODE[Repositórios de Código]:::internet
        
        subgraph OSINT_Comportamental [Inteligência de Crimes e Sociedade]
            VIOL[Violência e Gênero]:::osint
            CORR[Corrupção Política]:::osint
            ABUSE[Maus-tratos e Abusos]:::osint
            PEDO[Pedofilia e Assédio]:::osint
        end
    end

    %% Núcleo Central
    KCore((KADMON CORE)):::core
    MemL[Memória Lateral Persistente / Grafos]

    %% Mapeamento de Entradas para o Núcleo
    RF --> KCore
    FW --> KCore
    DC --> KCore
    SW --> KCore
    ...
    %% Saídas e Correlação
    subgraph Analise_Tatica [Correlação Tática]
        ATK[Análise de Ataques Ativos]
        EVAS[Simulação APT e Evasão]
    end

    subgraph Resposta_Soberana [Orquestração de Resposta]
        DYN[Defesa Ativa / Mitigação Dinâmica]:::action
        PAY[Geração de Payload / Exploit]:::action
    end
